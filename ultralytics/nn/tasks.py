...
def parse_model(d, ch, verbose=True):
    ...
    base_modules = frozenset(
        {
            ...
            SCDown,
            C2fCIB,
            A2C2f, Pzconv, FCM_3, FCM_2, FCM_1, FCM, Down, C3, FEM, CIM, LargeKernelBlock, SAM
        }
    )
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        m = (
            getattr(torch.nn, m[6:])
            if "nn." in m
            else getattr(__import__("torchvision").ops, m[15:])
        ...
        if m in base_modules:
            ...
            if m is C2fAttn:  # set 1) embed channels and 2) num heads
                ...
                args[4] = int(max(round(min(args[2], max_channels // 2 // 32)) * width, 1) if args[2] > 1 else args[2])
