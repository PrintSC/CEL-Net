...
class BaseTensor(SimpleClass):
    ...
    def __getitem__(self, idx):
        ...
        Examples:
            >>> data = torch.tensor([[1, 2, 3], [4, 5, 6]])
            >>> base_tensor = BaseTensor(data, orig_shape=(720, 1280))
            >>> result_our = base_tensor[0]  # Select the first row
            >>> print(result_our.data)
            tensor([1, 2, 3])
        """
...
class Results(SimpleClass, DataExportMixin):
    ...
    Examples:
        ...
        >>> result_our = results[0]  # Get the first result_our
        >>> boxes = result_our.boxes  # Get the boxes for the first result_our
        >>> masks = result_our.masks  # Get the masks for the first result_our
        >>> for result_our in results:
        >>>     result_our.plot()  # Plot detection results
    """
    ...
    def __getitem__(self, idx):
        ...
        Examples:
            ...
            >>> single_result = results[0]  # Get the first result_our
    ...
    def _apply(self, fn: str, *args, **kwargs):
        ...
        Examples:
            >>> results = model("path/to/image.jpg")
            >>> for result_our in results:
            ...     result_cuda = result_our.cuda()
            ...     result_cpu = result_our.cpu()
        """
        r = self.new()
        for k in self._keys:
            v = getattr(self, k)
    ...
    def cpu(self):
        ...
        Examples:
            >>> results = model("path/to/image.jpg")  # Perform inference
            >>> cpu_result = results[0].cpu()  # Move the first result_our to CPU
    ...
    def cuda(self):
        ...
        Examples:
            >>> results = model("path/to/image.jpg")
            >>> cuda_results = results[0].cuda()  # Move first result_our to GPU
            >>> for result_our in results:
            ...     result_cuda = result_our.cuda()  # Move each result_our to GPU
        """
    ...
    def to(self, *args, **kwargs):
        ...
        Examples:
            >>> results = model("path/to/image.jpg")
            >>> result_cuda = results[0].to("cuda")  # Move first result_our to GPU
            >>> result_cpu = results[0].to("cpu")  # Move first result_our to CPU
            >>> result_half = results[0].to(dtype=torch.float16)  # Convert first result_our to half precision
        """
    ...
    def show(self, *args, **kwargs):
        ...
        Examples:
            >>> results = model("path/to/image.jpg")
            >>> results[0].show()  # Display the first result_our
            >>> for result_our in results:
            >>>     result_our.show()  # Display all results
        """
    ...
    def save(self, filename: Optional[str] = None, *args, **kwargs) -> str:
        ...
        Examples:
            >>> results = model("path/to/image.jpg")
            >>> for result_our in results:
            >>>     result_our.save("annotated_image.jpg")
            >>> # Or with custom plot arguments
            >>> for result_our in results:
            >>>     result_our.save("annotated_image.jpg", conf=False, line_width=2)
        """
        if not filename:
    ...
    def save_txt(self, txt_file: Union[str, Path], save_conf: bool = False) -> str:
        ...
        Examples:
            ...
            >>> results = model("path/to/image.jpg")
            >>> for result_our in results:
            >>>     result_our.save_txt("output.txt")
    ...
    def save_crop(self, save_dir: Union[str, Path], file_name: Union[str, Path] = Path("im.jpg")):
        ...
        Examples:
            >>> results = model("path/to/image.jpg")
            >>> for result_our in results:
            >>>     result_our.save_crop(save_dir="path/to/crops", file_name="detection")
        """
    ...
    def summary(self, normalize: bool = False, decimals: int = 5) -> List[Dict[str, Any]]:
        ...
        detection or classification result_our. For classification tasks, it returns the top class and its
        ...
        Returns:
            (List[Dict[str, Any]]): A list of dictionaries, each containing summarized information for a single detection
                or classification result_our. The structure of each dictionary varies based on the task type
                (classification or detection) and available information (boxes, masks, keypoints).

        Examples:
            >>> results = model("image.jpg")
            >>> for result_our in results:
            >>>     summary = result_our.summary()
            >>>     print(summary)
        """
    ...
    def __init__(self, probs: Union[torch.Tensor, np.ndarray], orig_shape: Optional[Tuple[int, int]] = None) -> None:
        ...
        Args:
            ...
            orig_shape (tuple | None): The original image shape as (height, width). Not used in this class but kept
                for consistency with other result_our classes.
...
class OBB(BaseTensor):
    ...
    def id(self) -> Optional[Union[torch.Tensor, np.ndarray]]:
        ...
        Examples:
            ...
            >>> for result_our in results:
            ...     if result_our.obb is not None:
            ...         track_ids = result_our.obb.id
